...
def plot_area(xarray, values, ax, label, color=None, ls='-', zorder=5): 
    ...
    ax.fill_between(xarray, below, above, lw=3.5, color=color, alpha=0.2)
    pass
...
ax.set_title(r'Ridge regression on diabetes dataset, $\lambda=0.1$', fontsize=14)
ax.set_yscale('log')
ax.set_xlim(0, 310)
...
fig.tight_layout()
fig.savefig('Figures/ridge_diabetes.pdf')
fig.show()
# %%
